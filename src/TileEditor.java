...
public class TileEditor extends JFrame implements KeyListener {
    ...
    private void saveTile(){
        source.getTiles().get(index).setWalls(north.getState(), south.getState(), east.getState(), west.getState());
    }
    ...
    private void makeNewTile(){
        ...
        try {
            ...
            if(tileImage.getWidth() == 32 && tileImage.getHeight() == 32) {
                ...
                index = source.getTiles().size() - 1;
                Tile currTile = source.getTiles().get(index);
    ...
    public TileEditor(Game source, int index, File path) {
        defaultPath = path;
